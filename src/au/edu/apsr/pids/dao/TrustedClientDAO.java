...
public class TrustedClientDAO
{
    ...
    private static final String SELECT_CLIENT_SQL = 
        ...
        "FROM trusted_client WHERE app_id = ? " +
        "AND ((shared_secret = ? AND shared_secret IS NOT NULL) OR (ip_address = ? AND ip_address IS NOT NULL))";
     ...
     * @exception DAOException
     */
    public TrustedClient retrieve(String appId, String ipAddress, String sharedSecret) throws DAOException
    {
        Connection c = null; 
        ...
        try
        {
            ...
            ps.setString(2, sharedSecret);
            ps.setString(3, ipAddress);
            log.info(SELECT_CLIENT_SQL);
            log.error("After : " + ps.toString());
            rs = ps.executeQuery();
            
            TrustedClient id = null;
            if (rs.next())
            {
                id = new TrustedClient(rs.getString("ip_address"),appId, rs.getString("description"),rs.getString("shared_secret"));
